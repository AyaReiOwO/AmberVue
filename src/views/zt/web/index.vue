...
const initChart = () => {
  ...
  */
  ...
  //1.中国地图
  var chinaMap = '/asset/get/s/china.json';
  ...
  const mapInit = () => {
    //调用中国地图（同步）
    api.axios.get(`/src/assets/mapJson/data-1621947890604-aSx9ZxNx1.json`).then(geoJson => {
      //获取到长度（每个省里有几个市）
        ...
        geo: [
          {
            ...
            itemStyle: {
              //地图区域的多边形 图形样式
              normal: {
                areaColor: 'rgb(0,83,211)', //地区颜色

                borderColor: '#fff',
                borderWidth: 0.5,
                opacity: 0.7,
                shadowColor: 'rgb(0,83,211)', //阴影颜色
...
let heightArr = reactive([]);

onMounted(() => {
  const width = window.innerWidth;
  const height = window.innerHeight;

  const diffHeight = Math.floor(height / 10);

  const rNum = Math.floor(width * 0.8);

  [1, 2, 3, 4, 5, 6, 7, 8].map(item => {
    console.log(parseInt(item) * diffHeight);
    heightArr.push(Math.floor(Math.sqrt(rNum * rNum - parseInt(item) * diffHeight * parseInt(item) * diffHeight)));
  });
...
<template>
  <div class="min-h-full min-w-full h-full flex justify-center">
    <div class="surround-box">
      <div v-for="item in 10" :key="item" class="bg-light-50">
        {{ item }}
      </div>
    </div>

    <div id="main" class="w-3/5 min-w-[960px] h-[720px]"></div>
